...
bool solveMazeUtil(int maze[N][N], int x, int y, int sol[N][N], int visited[N][N])
{
    ...
    if (isSafe(maze, x, y, visited))
    {
        ...
        if (solveMazeUtil(maze, x, y+1, sol, visited)) // right
            return true;

        if (solveMazeUtil(maze,x+1, y, sol, visited)) // down
            return true;

        if (solveMazeUtil(maze, x, y-1, sol, visited)) // left
            return true;

        if (solveMazeUtil(maze, x-1, y, sol, visited)) // up
            return true;
